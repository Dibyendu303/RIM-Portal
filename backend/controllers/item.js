...
//   }
  ...
  const promises = itemsList?.map(async (data) => {
    const newItem = new Item({
      ...
      itemDocument: new mongoose.Types.ObjectId("643061d763c19c934681032a"), // Assigning an ObjectId to itemDocument field
      status: data.status,
  ...
  try {
    ...
    res.status(201).send({ result: "Success", items: addedItems, itemDocument: savedItemDocument });
  } catch (err) {
    res.status(500).send(err);
    console.log(err);
  }
